...
//if(parameter('date') || parameter('cardmode')){
if(1 !== 1){
    var countdowndatalink = window.location.search.substring(1)
    ...
    if (typeof window.supabaseClient !== "undefined" && window.supabaseClient.auth) {
        window.supabaseClient.auth.getSession().then(({ data: { session } }) => {
            if (!session?.user) {
                if(parameter('id')){
                    //get countdown data from database
                    loadCountdownFromDatabase(parameter('id')).then((countdownData) => {
                        if (countdownData) {
                            if (countdownData.visibility === 0) {
                                // Private countdown - prompt for login immediately
                                showToast('This countdown is private. Please log in to view it.', 'error');
                            } else {
                                // Public countdown - process it info pane
                                countdowndatalink = countdownData.data;
                                alert(countdowndatalink);
                                processURLParameters();
                            }
                        } else {
                            // Countdown not found
                            showToast('Countdown not found.', 'error');
                            processURLParameters();
                        }
                    });
                }
                else{
                    if(localStorage.getItem("preloginlocalcountdownsave")){
                        var countdowndatalink = localStorage.getItem("preloginlocalcountdownsave");
                        processURLParameters();

                    }
                    else{
                        //init with timekeeper
                        var countdowndatalink = '';
                        processURLParameters();
                ...
                if(parameter('id')){
                    ...
                    loadCountdownFromDatabase(parameter('id')).then((countdownData) => {
                        if (countdownData) {
                            alert(countdownData.visibility);
                            alert(countdownData.data);
                            if (countdownData.visibility === 0) {
                                // Private countdown - check if user has access
                                if (countdownData.creator === session.user.id || 
                                    (countdownData.collaborator_ids && countdownData.collaborator_ids.includes(session.user.id))) {
                                    // User has access - process the countdown
                                    countdowndatalink = countdownData.data;
                                    alert(countdowndatalink);
                                    processURLParameters();
                                } else {
                                    // User doesn't have access - show restricted page
                                    showToast('You do not have access to this private countdown.', 'error');
                                }
                            } else {
                                // Public countdown - process it info pane
                                countdowndatalink = countdownData.data;
...
// Load countdown data from database based on ID
async function loadCountdownFromDatabase(countdownId) {
    if (!countdownId) {
        return null;
    }
    
    try {
        console.log('[loadCountdownFromDatabase] Loading countdown with ID:', countdownId);
        
        const { data, error } = await supabaseClient
            .from('countdown')
            .select('data, visibility, creator, collaborator_ids')
            .eq('id', countdownId)
            .single();
            
        if (error) {
            console.error('[loadCountdownFromDatabase] Error loading countdown:', error.message);
            return null;
        }
        
        if (!data) {
            console.log('[loadCountdownFromDatabase] Countdown not found');
            return null;
        }
        
        console.log('[loadCountdownFromDatabase] Raw data from database:', data);
        console.log('[loadCountdownFromDatabase] Data field type:', typeof data.data);
        console.log('[loadCountdownFromDatabase] Data field value:', data.data);
        
        if (!data.data) {
            console.error('[loadCountdownFromDatabase] Data field is null/undefined');
            return null;
        }
        
        // Parse the countdown data and return with metadata
        const countdownData = JSON.parse(data.data);
        console.log('[loadCountdownFromDatabase] Parsed countdown data:', countdownData);
        
        return {
            data: data.data, // Return the raw JSON string as data field
        ...
        console.error('[loadCountdownFromDatabase] Error during database load:', error);
        return null;
    }
}
...
function cardmodemanager(){
  ...
  function processURLParameters() {
    alert(variedparameter('date'));
      ...
      if(variedparameter("confettitype")){
          confettiType = decodeURIComponent(variedparameter("confettitype")); //if the confetti type exists, set it
      }
      else{
          confettiType = "1";
      }
  
  if(variedparameter('progress')){
      document.querySelector('.progressdatepicker').value = variedparameter('progress');
// Check if progress date is before countdown end date
const progressDate = new Date(variedparameter('progress')).getTime();
const countdownDate = new Date(variedparameter('date')).getTime();

if(variedparameter('progressposition') && variedparameter('progressposition') !== "null"){
    if(variedparameter('progressposition') == 'bar'){
    ...
    }else if(variedparameter('progressposition') == 'fs'){
...
if(!variedparameter('schedule')){
  ...
  progressbarposition = variedparameter('progressposition');
        if(variedparameter('progressposition')){
        if(variedparameter('progressposition') == 'bar'){
      ...
      if (variedparameter('atc')) {
          bgstring = variedparameter('atc'); //if there is an animated text countdown background, set that to bg string
      }
      else {
          bgstring = "none"; //hypothetically not necessary since that's already the value but there just in case
      }
  
      if(variedparameter('endingsound')){
          document.getElementById("audioLink").value = atob(variedparameter('endingsound'));
      }
  ...
  const hasAnyColorParams = legacyParams.some(({param}) => variedparameter(param)) || 
      Array.from({length: 4}, (_, i) => i + 5).some(i => variedparameter(`color${i}`));
  ...
  if (!hasAnyColorParams) {
      ...
      legacyParams.forEach(({param, id, cssVar}) => {
        if (variedparameter(param)) {
            if (variedparameter(param) === "null") {
                console.log(`Colors(${id.slice(-1)}) could not be imported properly.`);
            } else {
                const paramValue = variedparameter(param);
                if (paramValue === 'fg') {
      ...
      for (let i = 5; i <= 8; i++) {
        const colorParam = variedparameter(`color${i}`);
    ...
    if (container) {
      ...
      if(variedparameter("schedule") !== "null" && variedparameter("schedule")){  
      ...
      //fonts or typefaces
      if (variedparameter('typeface')) {
          if (decodeURIComponent(variedparameter('typeface')) == "Fredoka One") { //decode URI Component simply replaces %20 with a space, etc
              FontFredoka('auto'); //calls the function for said font
          }
          else if (decodeURIComponent(variedparameter('typeface')) == "Poppins") {
              FontPoppins('auto');
          }
          else if (decodeURIComponent(variedparameter('typeface')) == "Yeseva One") {
              FontDMSerif('auto'); //fallback for old depreciated font option Yeseva, replaced by DM Serif Display
          }
          else if (decodeURIComponent(variedparameter('typeface')) == "DM Serif Display") {
              FontDMSerif('auto');
          }
          else if (decodeURIComponent(variedparameter('typeface')) == "Orbitron") {
              FontMichroma('auto');
          }
          else if (decodeURIComponent(variedparameter('typeface')) == "Michroma") {
            FontMichroma('auto');
        }
      }
      else {
      ...
      handleTitleNavigation();
          ...
          //date
          if (variedparameter('date')) {
              if(variedparameter('date').includes('T') && variedparameter('date').includes(':') ){ //if there is an included time, it will be saved as 12/34/56T12:34, this is checking for the T and the :
                 document.querySelector(".datepicker").value = variedparameter('date'); 
              }
              else{
                  document.querySelector(".datepicker").value = variedparameter('date') + 'T00:00'; //if there is no included time, and it's just 12/34/56 for example, it adds the T00:00 for midnight. backwards compatibility for before time was supported
              }
    ...
    if ((savedLinks) && (savedLinks !== '[]' && savedLinks !== '' && savedLinks !== 'null') && !parameter("createnew")) { // if countdowns have been saved
      ...
      if ((window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) || variedparameter("atc") !== "none") {
      ...
      //countdown title
      if (variedparameter('title')) { 
          document.getElementById("countdowntitle").value = decodeURIComponent(variedparameter('title')); //set the front-facing countdown title input to the parameter value, decoding all %20s and such
          ...
          document.querySelector('meta[property="og:title"]').setAttribute('content', 'Countdown to ' + decodeURIComponent(variedparameter('title')));
      }
  
  
      //bg again this time to finish the job
      var bgstring = "none";
      if (variedparameter('atc') && variedparameter('atc') != "none" && variedparameter('atc') != "undefined") { //checking it has a good value
          setbg(variedparameter('atc'), 'auto'); //setbg function takes the number and sets it to that bg, auto tells it that it was run not from settings and not to setcountdowngeneral
          bgstring = variedparameter('atc'); //set the var to the param
      }
      else {
          bgstring = 'none';
          ...
          if(variedparameter("schedule") !== "null" && variedparameter("atc") == "none"){
      ...
      if(confettiType == "1"){
          ...
          ConfettiModeEmoji();
          document.getElementById("confettiEmojiPicker").value = decodeURIComponent(variedparameter("confettitype"));
      }
...
document.addEventListener('DOMContentLoaded', initFloatingIcons);
          ...
          for (let attempt = 0; attempt < maxAttempts; attempt++) {
              ...
              try {
                  ...
                  if (error && error.code === 'PGRST116') {
                      // No rows returned, ID is unique
                      return result;
                  } else if (error) {
                      console.error('[generateShortId] Database check failed:', error.message);
                      // If we can't check the database, return the generated ID anyway
                      return result;
                  } else if (data) {
                      // ID exists, try again
                      console.log(`[generateShortId] ID collision detected: ${result}, regenerating...`);
                      continue;
                  }
              } catch (error) {
                  console.error('[generateShortId] Error checking ID uniqueness:', error);
                  // If we can't check the database, return the generated ID anyway
                  return result;
              }
          }
          
          // If we've exhausted all attempts, generate a longer ID to reduce collision probability
          console.warn('[generateShortId] Max attempts reached, generating longer ID');
          let result = '';
          for (let i = 0; i < 16; i++) {
              result += chars.charAt(Math.floor(Math.random() * chars.length));
          }
      ...
      async function syncCountdownToDatabase() {
          ...
          try {
              ...
              // Get colors in the same format as setCountDowngeneral
              const colorsNormalized = [];
              const colorPickers = document.querySelectorAll('.colorpicker, .disabledcolorpicker');
              colorPickers.forEach((picker, index) => {
                  if (picker.dataset.useThemeColor === 'true') {
                      colorsNormalized[index] = 'fg';
              ...
              // Build color parameters string (same as setCountDowngeneral)
              let colorParams = '';
              colorsNormalized.forEach((color, index) => {
                  if (index < 4) {
                      // Maintain backwards compatibility with original parameter names
              ...
              // Build parameter string in the same format as setCountDowngeneral
              const paramString = '?date=' + document.querySelector(".datepicker").value + 
                  colorParams + 
                  '&typeface=' + encodeURIComponent(getComputedStyle(document.documentElement).getPropertyValue('--typeface')) + 
                  '&atc=' + bgstring + 
              ...
              const countdownId = await generateShortId();
              ...
              if (error) {
                  ...
                  // Update URL with countdown ID
                  const newUrl = window.location.protocol + "//" + window.location.host + window.location.pathname + 
                      '?id=' + countdownId;
                  window.history.replaceState({ path: newUrl }, '', newUrl);
                  console.log('[syncCountdownToDatabase] URL updated with countdown ID:', countdownId);
              }
          } catch (error) {
      ...
      function variedparameter(name){
        if(countdowndatalink){
            var query = countdowndatalink;
        }
        else{
            return null;
        }
        
        // Check if the data is JSON format (starts with {)
        if (query.trim().startsWith('{')) {
            try {
                const jsonData = JSON.parse(query);
                return jsonData[name] || null;
            } catch (error) {
                console.error('[variedparameter] Error parsing JSON:', error);
                return null;
            }
        }
        
        // Handle URL-style parameters (key=value&key2=value2)
        var parameters = query.split('&');
        for (var i = 0; i < parameters.length; i++) {
            var pair = parameters[i].split('=');
            if (pair[0] == name) {
