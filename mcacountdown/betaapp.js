    ...
    ProgressPositionBar('manual');
      ...
      function schedule_resettimeinputs(){
        ...
        function parseICSDate(dateStr) {
          ...
          function schedule_parseTimeToMinutes(time) {
              const [hours, minutes] = time.split(':').map(Number);
              return (hours * 60) + minutes;
          }

          function schedule_minutesToTime(minutes) {
          ...
          function schedule_roundMinutesForGrid(minutes) {
              return Math.max(0, Math.min(1380, Math.round(minutes / 30) * 30));
          }

          function schedule_formatHourLabel(hour) {
              const suffix = hour >= 12 ? 'PM' : 'AM';
              const displayHour = hour % 12 || 12;
              return `${displayHour} ${suffix}`;
          }

          function schedule_buildTimeAxis() {
              const timeAxis = document.getElementById('schedule-timeAxis');
              if (!timeAxis) {
                  return;
              }

              timeAxis.innerHTML = '<div class="schedule-time-axis-spacer"></div>' + Array.from({ length: 24 }, (_, hour) => `
                  <div class="schedule-time-axis-slot" style="height:${schedule_hourHeight}px">
                      <span>${schedule_formatHourLabel(hour)}</span>
                  </div>
              `).join('');
          }
          ...
          function schedule_prepareSlotTarget(day, minutes = null) {
              schedule_editingEvent = null;
              schedule_editingExceptionDay = day !== null ? String(day) : null;
              schedule_isPickingExceptionDay = false;

              if (minutes !== null) {
                  const startMinutes = schedule_roundMinutesForGrid(minutes);
                  const endMinutes = Math.min(startMinutes + 60, 1439);
                  document.getElementById('schedule-startTime').value = schedule_minutesToTime(startMinutes);
                  document.getElementById('schedule-endTime').value = schedule_minutesToTime(endMinutes);
              }
          ...
          function schedule_handleGridSlotClick(mouseEvent, day = null, displayDay = null) {
              ...
              if (mouseEvent.target.closest('.schedule-calendar-event')) {
                  return;
              }

              const grid = mouseEvent.currentTarget;
              const gridRect = grid.getBoundingClientRect();
              const offsetY = Math.max(0, Math.min(gridRect.height, mouseEvent.clientY - gridRect.top));
              const minutes = schedule_roundMinutesForGrid((offsetY / gridRect.height) * schedule_minutesPerDay);
          ...
          function schedule_editEvent(event, isException = false, day = null) {
            if (!event && isException) {
                const existingStartTime = document.getElementById('schedule-startTime').value;
                schedule_prepareSlotTarget(day, existingStartTime ? schedule_parseTimeToMinutes(existingStartTime) : 540);
                document.getElementById("schedule-eventTitle").scrollIntoView({ behavior: 'smooth', block: 'center' });
                showToast(`${schedule_getDayName(day)} is selected. Click a time slot or save the event above.`, 'info');
                return;
            }
              schedule_editingEvent = event;
              schedule_editingExceptionDay = isException ? String(day) : null;
              schedule_isPickingExceptionDay = false;
          ...
          function schedule_addExceptionDay(day = null) {
              ...
              if (selectedDay === null || typeof selectedDay === 'undefined') {
                  showToast('Pick a day from the add-exception column', 'error');
                  return;
              }
              ...
              schedule_updateURL();
              schedule_updateScheduleViewer();
              schedule_prepareSlotTarget(selectedDay, document.getElementById('schedule-startTime').value ? schedule_parseTimeToMinutes(document.getElementById('schedule-startTime').value) : 540);

              const newDayColumn = document.querySelector(`.schedule-calendar-day-exception[data-day-key="${selectedDay}"]`);
              if (newDayColumn) {
                  newDayColumn.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
              }
          }
          ...
          function schedule_renderEventCard(event, editAction, removeAction) {
              const startMinutes = schedule_parseTimeToMinutes(event.startTime);
              ...
              return `
                  <div class="schedule-calendar-event${sizeClass}" style="top:${top}px; height:${height}px;" onclick="${editAction}">
                      ...
                      <div class="schedule-calendar-event-actions">
                          <a href="#" class="schedule-calendar-event-action" onclick="event.preventDefault(); event.stopPropagation(); ${editAction}; return false;"><i class="fa-solid fa-pencil"></i></a>
                          <a href="#" class="schedule-calendar-event-action warning" onclick="event.preventDefault(); event.stopPropagation(); ${removeAction}; return false;"><i class="fa-solid fa-trash"></i></a>
                      </div>
                  </div>
              `;
          }
          ...
          function schedule_renderDayGrid(events, day = null, displayDay = null) {
              ...
              return `
                  <div class="schedule-calendar-grid" onclick="schedule_handleGridSlotClick(event, ${gridTarget}, ${displayDayTarget})" style="height:${schedule_hourHeight * 24}px;">
                      ...
                      ${events.length ? '' : '<div class="schedule-calendar-empty">Click anywhere in this day to add an event.</div>'}
                  </div>
              `;
          }
          ...
          function schedule_updateAddDayOptions() {
              ...
              if (!availableDays.length) {
                  addDayOptions.innerHTML = '<div class="schedule-calendar-add-empty">Every weekday already has its own exception column.</div>';
                  return;
              }

              addDayOptions.insertAdjacentHTML(
                  'beforeend',
                  '<p class="schedule-calendar-add-copy">Choose a day, then click into that column to place events on the grid.</p>'
              );

              availableDays.forEach(day => {
                  addDayOptions.insertAdjacentHTML(
                      'beforeend',
                      `<button type="button" class="schedule-calendar-add-choice" onclick="schedule_addExceptionDay('${day}')"><i class="fa-solid ${schedule_getDayIcon(day)}"></i> ${schedule_getDayName(day)}</button>`
                  );
              });
          }
    ...
    if ((event.ctrlKey || event.metaKey) && event.key === 's') {
      ...
      function removeToast(toast) {
        ...
        function setLightMode() {
            ...
            document.documentElement.style.setProperty('--inputbackground', 'radial-gradient(circle at 12% -35%, rgba(255, 255, 255, 0.98), transparent 46%), radial-gradient(circle at 108% 125%, rgba(132, 38, 255, 0.18), transparent 54%), linear-gradient(145deg, rgba(255, 255, 255, 0.82), rgba(246, 240, 255, 0.66))');
            document.documentElement.style.setProperty('--inputborder', 'rgba(86, 56, 125, 0.16)');
            document.documentElement.style.setProperty('--inputshadow', '0 16px 38px rgba(61, 38, 89, 0.1), inset 0 1px 0 rgba(255, 255, 255, 0.92)');
            document.documentElement.style.setProperty('--inputhoverborder', 'rgba(132, 38, 255, 0.42)');
            document.documentElement.style.setProperty('--inputplaceholder', 'rgba(28, 18, 42, 0.46)');
            document.documentElement.style.setProperty('--control-color-scheme', 'light');
            document.documentElement.style.setProperty('--calendar-picker-filter', 'opacity(0.62)');
            ...
            document.documentElement.style.setProperty('--schedulecalendarsurface', 'rgba(255, 255, 255, 0.68)');
            document.documentElement.style.setProperty('--schedulecalendarheader', 'rgba(255, 255, 255, 0.9)');
            document.documentElement.style.setProperty('--schedulecalendarcell', 'rgba(250, 248, 255, 0.62)');
            document.documentElement.style.setProperty('--schedulecalendaraxis', 'rgba(247, 243, 252, 0.88)');
            document.documentElement.style.setProperty('--schedulecalendargridline', 'rgba(63, 42, 88, 0.1)');
            document.documentElement.style.setProperty('--schedulecalendarsubgridline', 'rgba(63, 42, 88, 0.045)');
            document.documentElement.style.setProperty('--schedulecalendarshadow', '0 24px 64px rgba(66, 42, 96, 0.12), inset 0 1px 0 rgba(255, 255, 255, 0.95)');
            document.documentElement.style.setProperty('--schedulecalendarscrollbar', 'rgba(84, 57, 117, 0.24)');
        ...
        function setDarkMode() {
            ...
            document.documentElement.style.setProperty('--inputbackground', 'radial-gradient(circle at 12% -35%, rgba(255, 255, 255, 0.15), transparent 46%), radial-gradient(circle at 108% 125%, rgba(132, 38, 255, 0.30), transparent 54%), linear-gradient(145deg, rgba(255, 255, 255, 0.055), rgba(132, 38, 255, 0.10))');
            document.documentElement.style.setProperty('--inputborder', 'rgba(255, 255, 255, 0.14)');
            document.documentElement.style.setProperty('--inputshadow', '0 12px 30px rgba(0, 0, 0, 0.16), inset 0 1px 0 rgba(255, 255, 255, 0.16)');
            document.documentElement.style.setProperty('--inputhoverborder', 'rgba(189, 145, 255, 0.48)');
            document.documentElement.style.setProperty('--inputplaceholder', 'rgba(255, 255, 255, 0.46)');
            document.documentElement.style.setProperty('--control-color-scheme', 'dark');
            document.documentElement.style.setProperty('--calendar-picker-filter', 'invert(1) opacity(0.72)');
            ...
            document.documentElement.style.setProperty('--schedulecalendarsurface', 'rgba(18, 18, 24, 0.72)');
            document.documentElement.style.setProperty('--schedulecalendarheader', 'rgba(20, 20, 26, 0.9)');
            document.documentElement.style.setProperty('--schedulecalendarcell', 'rgba(10, 10, 15, 0.34)');
            document.documentElement.style.setProperty('--schedulecalendaraxis', 'rgba(14, 14, 19, 0.74)');
            document.documentElement.style.setProperty('--schedulecalendargridline', 'rgba(255, 255, 255, 0.08)');
            document.documentElement.style.setProperty('--schedulecalendarsubgridline', 'rgba(255, 255, 255, 0.035)');
            document.documentElement.style.setProperty('--schedulecalendarshadow', '0 22px 60px rgba(0, 0, 0, 0.24), inset 0 1px 0 rgba(255, 255, 255, 0.12)');
            document.documentElement.style.setProperty('--schedulecalendarscrollbar', 'rgba(255, 255, 255, 0.18)');
